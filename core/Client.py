...
from core.function import (
    gather_flat_grad,
    get_trainable_hyper_params,
    loss_adjust_cross_entropy,
...
class Client:
    ...
    def __init__(self,
        ...
        self.ldr_train = DataLoader(
            DatasetSplit(dataset, idxs[client_id]),
            batch_size=self.args.local_bs,
            shuffle=True,
        )
        self.ldr_val = DataLoader(
            DatasetSplit(dataset, idxs[-client_id - 1]),
            batch_size=self.args.local_bs,
            shuffle=True,
        )
        ...
        self.hyper_optimizer = SGD(
            [self.hyper_param[k] for k in self.hyper_param], lr=args.hlr)
    ...
    def grad_d_in_d_y(self, create_graph=True):
        if hasattr(self, 'net0'):
            del self.net0
        self.net0 = copy.deepcopy(self.net)
        self.net0.train()
        ...
        for batch_idx, (images, labels) in enumerate(self.ldr_train):
            ...
            log_probs = self.net0(images)
            loss = self.loss_func(log_probs, labels)
            d_in_d_y += gather_flat_grad(
                grad(loss, self.net0.parameters(), create_graph=create_graph))
        d_in_d_y /= batch_idx + 1.0
        return d_in_d_y

    def grad_d_out_d_y(self, create_graph=True):
        if hasattr(self, 'net0'):
            del self.net0
        self.net0 = copy.deepcopy(self.net)
        self.net0.train()
        ...
        for batch_idx, (images, labels) in enumerate(self.ldr_val):
            ...
            d_out_d_y += gather_flat_grad(
                grad(loss, self.net0.parameters(), create_graph=create_graph))
        d_out_d_y /= batch_idx + 1.0
        return d_out_d_y

    def s_func(self, v):
        d_out_d_y = self.grad_d_out_d_y(create_graph=False)
        ...
        hessian_term = gather_flat_grad(
            grad(
                ...
                grad_outputs=v.view(-1),
                retain_graph=False,
            ))
        hessian_term.detach()
        d_in_d_y.detach()
        d_out_d_y.detach()
        return [d_in_d_y, hessian_term - d_out_d_y]
    ...
    def hvp_iter(self, p, lr):
        ...
        hessian_term = gather_flat_grad(
            grad(
                self.d_in_d_y,
                self.net0.parameters(),
                grad_outputs=self.counter.view(-1),
                retain_graph=True,
            ))
    ...
    def grad_d_out_d_x(self, hyper_param=None, create_graph=True):
        if hyper_param == None:
        ...
        for batch_idx, (images, labels) in enumerate(self.ldr_val):
            ...
            d_out_d_x += gather_flat_grad(
                grad(loss,
                     get_trainable_hyper_params(hyper_param),
                     create_graph=True))
        d_out_d_x /= batch_idx + 1.0
        return d_out_d_x
    ...
    def hyper_grad(self, p):
        ...
        try:
            direct_grad = self.grad_d_out_d_x(create_graph=False)
    ...
    def hyper_update(self, hg):
        ...
        return -gather_flat_hyper_params(
            self.hyper_param) + gather_flat_hyper_params(self.hyper_param_init)

    def hyper_svrg_update(self, hg):
        try:
            direct_grad = self.grad_d_out_d_x()
            direct_grad_0 = self.grad_d_out_d_x(
                hyper_param=self.hyper_param_init)
