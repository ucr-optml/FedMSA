...
def neumann_hyperstep_preconditioner(
    d_val_loss_d_theta, d_train_loss_d_w, elementary_lr, num_neumann_terms, model
):
...
def gather_flat_hyper_params(params):
    if isinstance(params, dict):
        return torch.cat(
            [params[k].view(-1) for k in params if params[k].requires_grad]
        )
    else:
        return torch.cat([k.view(-1) for k in params if k.requires_grad])


def gather_flat_weight(params):
    if isinstance(params, dict):
        return torch.cat([params[k].view(-1) for k in params])
    else:
        return torch.cat([k.view(-1) for k in params])
...
def assign_weight_value(params, vector_value, requires_grad=False):
    i = 0
    max_len = vector_value.shape[0]
    if isinstance(params, dict):
        for k in params:
            if params[k].requires_grad or not requires_grad:
                num = params[k].nelement()
                value = vector_value[i : min(i + num, max_len)].clone()
                torch.reshape(value, params[k].shape)
                params[k] = value.view(params[k].shape)
                i += num
    else:
        for para in params:
            if para.requires_grad or not requires_grad:
                num = para.nelement()
                value = vector_value[i : min(i + num, max_len)].clone()
                para = value.view(para.shape)
                i += num
    return params
